...
import { compareParityTrees, normalizeParityTree } from "../src/deployment-v2/parity.js";

const fleetPath = "test/fixtures/deployment-v2/import/fleet.yaml";
const goldenFluxTree = "fixtures/deployment-v2/golden/cluster/flux";

function tempDir() {
  return mkdtempSync(join(tmpdir(), "deploy-v2-golden-"));
}
...
test("imported v1 golden tree compiles through deploy-v2 and records honest parity subset", () => {
  const importedDir = tempDir();
  const renderedDir = tempDir();

  const imported = importFleetV1({
    fleetPath,
...
test("render-flux check detects drift in compiled deploy-v2 tree", async () => {
  const repo = tempDir();
  importFleetV1({
    fleetPath,
    fluxTreePath: goldenFluxTree,

...
import { renderKubernetes as renderLiveKubernetes } from "../src/adapters/kubernetes.js";
import { buildProjectModel, projectModelToAdapterContext } from "../src/deployment/model.js";
import { loadYamlDocument } from "../src/deployment/io.js";
import { renderKubernetes } from "../src/deployment/render/kubernetes.js";
import { renderWorkloads } from "../src/deployment/render/workloads.js";
...
test("renderKubernetes mirrors live workload output for deployment fixture", () => {
...
test("renderKubernetes renders GPU placement as node-id affinity and device limits", () => {
  const deploymentInput = loadYamlDocument(fixture("deployment.yml"));
  deploymentInput.spec.workloads["assistant-api"].placement.gpu = {
    count: 1,
    vendor: "nvidia",
    class: "transcode",
    minMemoryMiB: 6144,
  ...
  const files = renderKubernetes(model).files;
  const deployment = docs(file(files, "apps/agents/assistant-api/deployment.yaml").content)[0];
  const expressions = deployment.spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution.nodeSelectorTerms[0].matchExpressions;

...
} from "../src/deployment-v2/model.js";
...
test("buildProjectModel parses fixture IR and exposes v1 adapter artifacts", () => {
  const model = buildProjectModel(input());
  const context = projectModelToAdapterContext(model);

  assert.equal(model.apiVersion, "deployment.jorisjonkers.dev/ir/v1");

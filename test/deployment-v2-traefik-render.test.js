...
import { renderTraefik } from "../src/deployment-v2/render/traefik.js";
...
test("deploy-v2 Traefik renders public and LAN IngressRoute files", () => {
...
test("deploy-v2 Traefik includes rule priority only in v2 render mode", () => {
  const result = renderTraefik(model({ renderMode: "v2" }));
  const publicRoute = docs(result.files[0])[0];

...
export type RenderMode = "parity" | "native";
...
export type ProjectModel = {
  apiVersion: "deployment.jorisjonkers.dev/ir";
...
const renderModeSchema = z.enum(["parity", "native"]);
...
export const placementModelSchema = z.object({
  ...
  gpu: z.object({
    ...
    minMemoryMiB: z.number().int().min(1).optional(),
    resourceName: z.string().optional(),
  }).optional(),
  eligibleNodeNames: z.array(z.string()).optional(),
  gpuResourceName: z.string().optional(),
...
export const projectModelSchema = z.object({
  apiVersion: z.literal("deployment.jorisjonkers.dev/ir"),
...
export function buildProjectModel(input: CompilerInputSet): ProjectModel {
  ...
  for (const workload of Object.values(workloads)) {
    const resolution = resolvePlacement(workload.placement, nodeContract);
    if (workload.placement.gpu) {
      workload.placement.eligibleNodeNames = resolution.nodeNames;
      workload.placement.gpuResourceName = resolution.gpuResourceName;
    }
  }
...
export function validateProjectModel(model: ProjectModel): Diagnostic[] {
  ...
  for (const [name, workload] of Object.entries(model.workloads)) {
    ...
    const placement = resolvePlacement(workload.placement, model.nodeContract);
    if (!placement.satisfiable) {
      const path = workload.placement.gpu ? pointer("workloads", name, "placement", "gpu") : pointer("workloads", name, "placement");
      diagnostic(diagnostics, "E_PLACEMENT_UNSATISFIABLE", path, `workload ${name} placement cannot match any active schedulable node`);
    }
  }
...
function normalizeWorkload(name: string, deploymentName: string, input: unknown): WorkloadModel {
  ...
  return workloadModelSchema.parse({
    ...
    placement: {
      ...
      gpu: value.placement?.gpu ? { count: 1, ...value.placement.gpu } : undefined,
...
function resolvePlacement(placement: PlacementModel, nodeContract: NodeContractModel): { satisfiable: boolean; nodeNames: string[]; gpuResourceName?: string } {
  const eligible = Object.entries(nodeContract.nodes).filter(([name, node]) => {
  ...
  const gpuEligible = placement.gpu
    ? eligible.filter(([, node]) => nodeGpuCount(node, placement.gpu!) >= placement.gpu!.count)
    : eligible;
  const nodeNames = gpuEligible.map(([name]) => name).sort();
  return {
    satisfiable: nodeNames.length > 0,
    nodeNames,
...
function nodeGpuCount(node: NodeModel, request: PlacementGpuModel): number {
  return (node.gpus ?? []).filter((gpu) => gpuMatchesRequest(gpu, request)).reduce((sum, gpu) => sum + (gpu.count ?? 1), 0);
}

function gpuMatchesRequest(gpu: GpuModel, request: PlacementGpuModel): boolean {
  if (request.vendor && gpu.vendor !== request.vendor) return false;
  if (request.model && gpu.model !== request.model) return false;
  if (request.class && gpu.class !== request.class) return false;
  if (request.resourceName && gpu.resourceName !== request.resourceName) return false;
  if (request.minMemoryMiB && gpu.memoryMiB < request.minMemoryMiB) return false;
  return true;
}

function gpuResourceNameFor(request: PlacementGpuModel, nodes: NodeModel[]): string | undefined {
  if (request.resourceName) return request.resourceName;
  const matchingGpus = nodes.flatMap((node) => (node.gpus ?? []).filter((gpu) => gpuMatchesRequest(gpu, request)));
  const resourceNames = [...new Set(matchingGpus.map((gpu) => gpu.resourceName).filter((value): value is string => Boolean(value)))].sort();
  if (resourceNames.length > 0) return resourceNames[0];
  const vendors = new Set(matchingGpus.map((gpu) => gpu.vendor));
  if (request.vendor === "nvidia" || (vendors.size === 1 && vendors.has("nvidia"))) return "nvidia.com/gpu";
  return undefined;
}

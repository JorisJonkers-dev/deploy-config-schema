...
The `flux-source` and `flux-packs` adapters need `flux-modules/packs/**`
...
`flux-modules` is already consumed by version tag/ref. Its release
workflow only echoes the tag, and existing JorisJonkers-dev GitHub workflow actions
consume flux-modules content by checking out that repository at a ref.
...
Consumers obtain packs by checking out `flux-modules` at a pinned tag or
...
provenance so generated output can be traced back to the flux-modules
...
3. Require the consumer to provide a pinned `flux-modules` checkout via
   an explicit root option.

## Rationale

Option 3 matches how `flux-modules` is already consumed: by tag/ref. It
also matches the existing GitHub workflow checkout pattern used by
JorisJonkers-dev/github-workflows actions for flux-modules content.
...
Consumers must check out or vendor `flux-modules` at the desired tag
